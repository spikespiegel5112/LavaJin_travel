...
<!-- /头部 -->
	...
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 首页管理 <span class="c-gray en">&gt;</span> 首页类别 <a id="reload" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:void(0);" onclick="location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="page-container">
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                <a href="javascript:;" onClick="category_edit('类别编辑','<?php echo U('system/index_category_edit');?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加类别</a>
            </span>
            ...
            <tbody>
            	<?php if(is_array($category_list)): $k = 0; $__LIST__ = $category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c">
                    ...
                    <td class="td-manage">
                        ...
                        <a title="编辑" href="javascript:;" onClick="category_edit('编辑','<?php echo U('system/index_category_edit',array('id'=>$vo['id']));?>','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
...
function category_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo U("system/index_category_enable");?>',
...
function category_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo U("system/index_category_delete");?>',
			data:'id='+id,
			dataType: 'json',
			success: function(data){
				if(data.status==1){
